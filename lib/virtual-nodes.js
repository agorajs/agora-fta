...
function createSeed(p) {
    return __assign(__assign({}, p), { width: 0, height: 0, label: 'seed', index: -1 });
}
...
function createVirtualNode(i, v, s) {
    ...
    return __assign(__assign(__assign({ index: i }, center), box), { origin: center, nodes: v, label: 'virtual', seed: s });
}
