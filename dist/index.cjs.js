...
function createVirtualNode(i, v, s) {
  ...
  if (s === void 0) if (agoraGraph.norm(borderNodes.top, topleft) > agoraGraph.norm(borderNodes.left, topleft)) {
    s = borderNodes.left;
  } else {
    s = borderNodes.top;
  }
...
function getBorders(borderNodes) {
  return {
    top: agoraGraph.top(borderNodes.top),
    right: agoraGraph.right(borderNodes.right),
    bottom: agoraGraph.bottom(borderNodes.bottom),
    left: agoraGraph.left(borderNodes.left)
  };
}
function getBorderNodes(nodes) {
  return {
    top: agoraGraph.top(nodes),
    //upmost node
    right: agoraGraph.right(nodes),
    // rightmost node
    bottom: agoraGraph.bottom(nodes),
    // bottomost node
    left: agoraGraph.left(nodes) // leftmost node
...
function D(v, q) {
  return filter_1(v, function (i) {
    return agoraGraph.top(i) >= agoraGraph.top(q);
  });
}
/* // Unused
export namespace D {
...
function L(v, q) {
  return filter_1(v, function (i) {
    return agoraGraph.left(i) < agoraGraph.left(q);
  });
}
/* // Unused
export namespace L {
...
function N(i, j, padding) {
  return i !== j && agoraGraph.overlap(i, j, {
    padding: padding
  });
}
/**
...
function TNN(V, q, d) {
  ...
  while (node = toVisit.pop()) {
    var _iterator = _createForOfIteratorHelper(NN(V, node, d)),
        _step;
 ...
 */
var forceTransfer = agoraGraph.createFunction(function (graph) {
  ...
  var _loop = function _loop() {
    ...
    _.forEach(virtuals, function (virtualNode) {
      ...
      _.forEach(virtualNode.nodes, function (i) {
        i.x += delta.x;
        i.y += delta.y;
      });
    });
  ...
  while (agoraGraph.hasOverlap(graph.nodes, {
    padding: -0.0001
  })) {
    _loop();
  }
...
function getVirtualNodes(G, d) {
  ...
  _.forEach(G.nodes, function (self) {
    var present = false;
...
var RightRemoval = {
  ...
  delta: function delta(i, j) {
    var delta = {
      x: agoraGraph.right(i) - agoraGraph.left(j),
      y: Math.min(Math.abs(agoraGraph.top(i) - agoraGraph.bottom(j)), Math.abs(agoraGraph.bottom(i) - agoraGraph.top(j)))
    };
...
var LeftRemoval = {
  ...
  delta: function delta(i, j) {
    var delta = {
      x: agoraGraph.right(j) - agoraGraph.left(i),
      y: Math.min(Math.abs(agoraGraph.top(i) - agoraGraph.bottom(j)), Math.abs(agoraGraph.bottom(i) - agoraGraph.top(j)))
    };
    if (delta.x <= delta.y) return delta.x;
    return 0;
...
var DownRemoval = {
  ...
  delta: function delta(i, j) {
    var delta = {
      x: Math.min(Math.abs(agoraGraph.left(i) - agoraGraph.right(j)), Math.abs(agoraGraph.right(i) - agoraGraph.left(j))),
      y: agoraGraph.bottom(i) - agoraGraph.top(j)
    };
...
var UpRemoval = {
  ...
  delta: function delta(i, j) {
    var delta = {
      x: Math.min(Math.abs(agoraGraph.left(i) - agoraGraph.right(j)), Math.abs(agoraGraph.right(i) - agoraGraph.left(j))),
      y: agoraGraph.bottom(j) - agoraGraph.top(i)
    };
    if (delta.y <= delta.x) return delta.y;
    return 0;
  },
...
function directedRemoveOverlap(nn, tnn, i, d, dir) {
  ...
  _.forEach(directedNN, function (j) {
    ...
    _.forEach(directedTNN, function (node) {
  ...
  _.forEach(directedTNN, function (j) {
    var jNN = dir.filter(NN(directedTNN, j, d), j);
    var jTNN = dir.filter(TNN(directedTNN, j, d), j); // console.log(j, jNN, jTNN)

    _.forEach(jNN, function (k) {
...
exports.ForceTransferAlgorithm = ForceTransferAlgorithm;
exports.default = ForceTransferAlgorithm;
exports.forceTransfer = forceTransfer;
